...
# from dev_daily_aqi_report.linear_approach import get_each_client
...
#     get_each_client([212, 219, 238, 35, 188])
...
def aqi_clients(client_id):
    ...
    params = {
        'where': f'aqithreshold is not null AND cid = {client_id}',
...
def aqi_today(formats, workbook, clientId, name, threshold, aqistates, f):
    ...
    row = 2
    logging.info("Today ----> %d", len(str(f)))
    hit_count = 0
...
def aqi_tomorrow(formats, workbook, clientId, name, threshold, aqistates,f, workbook_filename):
    ...
    hit_count = 0
    logging.info("Tomorrow ----> %d", len(str(f)))
    for i in range(len(f)):
